...
> (`ARCH_PORTDUINO_WASM`, built as a normal PlatformIO env — `pio run -e native-wasm` via
...
npm run build:wasm                     # -> web/dist/meshnode.{mjs,wasm} (runs firmware's pio run -e native-wasm)
```
