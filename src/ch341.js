...
export class CH341 {
  ...
  static async request({ vid = proto.VID, pid = proto.PID } = {}) {
    if (!("usb" in navigator)) throw new Error("WebUSB unavailable (use Chromium over https/localhost)");
  ...
  async transceive(writeBytes) {
    const data = writeBytes instanceof Uint8Array ? writeBytes : Uint8Array.from(writeBytes);
    ...
    try {
      ...
      for (const pkt of packets) {
        ...
        while (got < dataLen) {
          ...
          if (r.length === 0) break;
          for (let i = 0; i < r.length; i++) read[off + got + i] = proto.reverseByte(r[i]);
          got += r.length;
        }
        off += dataLen;
      }
